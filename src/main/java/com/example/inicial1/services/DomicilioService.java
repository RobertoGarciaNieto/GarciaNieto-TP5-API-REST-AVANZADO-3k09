...
public class DomicilioService implements BaseService<Domicilio>{
    ...
    public List<Domicilio> findAll() throws Exception {
        try {
            List<Domicilio> entities = domicilioRepository.findAll(); //Obtiene de la BD todas las personas que tengamos registradas
            return entities;
        } catch (Exception e) {
    ...
    public Domicilio findById(Long id) throws Exception {
        try {
            Optional<Domicilio> entityOptional =domicilioRepository.findById(id);  //Se usa porque no sabemos si en la BD va a haber una entidad con el id
